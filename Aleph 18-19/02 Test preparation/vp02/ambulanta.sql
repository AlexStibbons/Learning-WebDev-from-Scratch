...
-- zadatak 1.a 
select ime_psa from pas order by ime_psa asc;

-- zadatak 1.b 
select ime_psa, ime_vlasnika, prezime_vlasnika
from pas
where prezime_vlasnika like 'Petrovic'
order by ime_psa asc;

-- zadatak 1.c
select tip_intervencije.naziv, pas.ime_psa, pas.ime_vlasnika, pas.prezime_vlasnika
from intervencija left join pas on intervencija.pas_id = pas.id
left join tip_intervencije on intervencija.tip_intervencije_id = tip_intervencije.id
order by pas.ime_psa asc;

-- zadatak 1.d 
select tip_intervencije.naziv, intervencija.datum from
intervencija left join tip_intervencije on intervencija.tip_intervencije_id = tip_intervencije.id
where intervencija.datum like '2017-11-%';

-- zadatak 1.e 
select pas.ime_psa, count(*) from
pas left join intervencija on intervencija.pas_id = pas.id 
left join tip_intervencije on intervencija.tip_intervencije_id = tip_intervencije.id
where tip_intervencije.naziv like 'vak%'
group by pas.ime_psa;
